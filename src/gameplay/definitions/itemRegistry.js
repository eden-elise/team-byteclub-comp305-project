...
export class HealthPotion extends Item {
  ...
  static animationCallback = createThrowAnimationCallback({
    itemImage: HealthPotion.data.spritePath,
  });
  constructor() {
...
export class RegenerationPotion extends Item {
  ...
  static animationCallback = createThrowAnimationCallback({
    itemImage: RegenerationPotion.data.spritePath,
  });
  constructor() {
  ...
  async execute(source, target, textbox) {
    ...
    textbox.addLogEntry(
      `${target.name} has regeneration for ${regenerationEffect.duration} turns!`
    );
  }
}
...
export class AdrenalinePotion extends Item {
  ...
  static animationCallback = createThrowAnimationCallback({
    itemImage: AdrenalinePotion.data.spritePath,
  });
  constructor() {
...
export class PoisonPotion extends Item {
  ...
  static animationCallback = createThrowAnimationCallback({
    itemImage: PoisonPotion.data.spritePath,
  });
  constructor() {
...
export class FirePotion extends Item {
  ...
  static animationCallback = createThrowAnimationCallback({
    itemImage: FirePotion.data.spritePath,
  });
  constructor() {
...
export class FreezePotion extends Item {
  ...
  static animationCallback = createThrowAnimationCallback({
    itemImage: FreezePotion.data.spritePath,
  });
  constructor() {
...
export class MysteryPotion extends Item {
  ...
  static animationCallback = createThrowAnimationCallback({
    itemImage: MysteryPotion.data.spritePath,
  });
  constructor() {
  ...
  async execute(source, target, textbox) {
    ...
    if (
      Math.random() <
      0.5 +
        (0.25 * (source.stats.LUCK - target.stats.LUCK)) / (source.stats.LUCK + target.stats.LUCK)
    ) {
      if (num <= 0.33) {
        target.heal(40);
